        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {media.map((item, index) => (
              <motion.div key={item.id} className="relative group rounded-xl overflow-hidden border border-white/10 cursor-pointer" onClick={() => setSelectedIndex(index)}>
                {item.file_type?.startsWith("video") ? (
                  <ReactPlayer url={item.file_url} width="100%" height="100%" controls playsinline />
                ) : (
